...
export default function AdminLayout({ children, userName, userRole, isPlatformAdmin }: AdminLayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const pathname = usePathname();

  const allNavItems = isPlatformAdmin 
    ? [...navigationItems, ...platformAdminItems]
    : navigationItems;
          ...
          <div className="border-t border-slate-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 overflow-hidden">
                <p className="truncate text-sm font-medium text-slate-900">{userName || 'Admin'}</p>
                <p className="truncate text-xs text-slate-500">{userRole || 'Administrator'}</p>
              </div>
            </div>
          </div>
        </div>
